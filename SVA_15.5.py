...
from PySide6.QtWidgets import (QButtonGroup,
    ...
    QSizePolicy, QMessageBox, QSplitter, QDialog, QTextEdit, QStyle, QHeaderView, QFormLayout 
)
...
class UpdateDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        # 1. Aviso
        warning_label = QLabel("<b>ATENÇÃO:</b> Conecte-se à internet para verificações das versões.")
        ...
        close_btn.setStyleSheet("QPushButton { background-color: #4b5563; color: white; border: none; padding: 8px 20px; border-radius: 5px; font-weight: bold; } QPushButton:hover { background-color: #374151; }")
        ...
        self.setStyleSheet("""
            ...
            QTableWidget { border: 1px solid #e2e8f0; border-radius: 5px; background-color: white; }
            QHeaderView::section { background-color: #f1f5f9; color: #1e293b; font-weight: bold; padding: 5px; border: 1px solid #e2e8f0; }
    ...
    def download_new_version(self):
        ...
        global REMOTE_VERSION, REMOTE_DOWNLOAD_URL  
        if not REMOTE_DOWNLOAD_URL:
            QMessageBox.critical(self, "Erro de Download", "URL de download não encontrada. Tente verificar a atualização novamente.")
            return
    ...
    def load_project(self): # Esta função agora é para o botão "Buscar Projeto"
        ...
        if file_path:
            try:
                ...
                QMessageBox.information(self, "Sucesso", f"Projeto '{project_data.get('test_name', 'Sem Nome')}' carregado com sucesso. Clique em 'Iniciar Análise' para continuar.")
    ...
    def load_selected_project(self, item):
        ...
        if os.path.exists(file_path):
            try:
                ...
                QMessageBox.critical(self, "Erro", f"Não foi possível ler o arquivo do projeto:\n{e}")
...
class AnalysisCard(QGroupBox):
    ...
    def generate_pdf_report(self):
        try:
            ...
            QMessageBox.critical(self, "Erro", f"Erro ao gerar o relatório PDF:\n{str(e)}")
...
class CalibrationWidget(QWidget):
    ...
    def calculate_and_apply(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Erro", f"Distância real inválida: {e}")
            return
        ...
        QMessageBox.information(self, "Sucesso", f"Calibração de {pixels_per_cm:.2f} pixels/cm aplicada com sucesso!")
...
class MainWindow(QWidget):
    ...
    def save_plantability_calculation(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Erro ao Salvar", f"Ocorreu um erro ao tentar salvar os dados na tabela:\n{e}")
    ...
    def calculate_plantability(self):
        ...
        try:
            ...
            self.last_plantability_results = None
            QMessageBox.critical(self, "Erro de Entrada", f"Por favor, insira valores numéricos válidos.\n\nDetalhe: {e}")
        except Exception as e:
            QMessageBox.critical(self, "Erro no Cálculo", f"Ocorreu um erro inesperado durante o cálculo:\n{e}")
    ...
    def clear_all_analyses(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            self.next_analysis_id = 1
            self.update_reports_display()
            QMessageBox.information(self, "Limpeza Concluída", "Todas as análises foram removidas.")
    ...
    def save_all_analyses_to_pdf(self):
        ...
        try:
            ...
            if filename:
                ...
                doc.build(story)
                QMessageBox.information(self, "Sucesso", f"Relatório consolidado salvo com sucesso em:\n{filename}")

        except Exception as e:
            QMessageBox.critical(self, "Erro", f"Erro ao gerar o relatório PDF consolidado:\n{str(e)}")
    ...
    def import_statistics_from_csv(self):
        ...
        try:
            ...
            QMessageBox.information(self, "Sucesso", f"{len(imported_analyses)} registros importados com sucesso do arquivo CSV.")

        except Exception as e:
            QMessageBox.critical(self, "Erro de Importação", f"Ocorreu um erro grave ao ler o arquivo CSV:\n{str(e)}\n\nVerifique se o arquivo está no formato correto (separado por ';').")
    ...
    def perform_statistical_analysis(self):
        ...
        try:
            ...
            try:
                ...
                QMessageBox.critical(self, "Erro de Cabeçalho", "Não foi possível encontrar as colunas 'Deliniamento' ou 'Vel. Esteira (km/h)' na tabela. A análise não pode continuar.")
                return
            # --- FIM DA CORREÇÃO ---
    ...
    def save_anova_results_to_pdf(self):
        ...
        try:
            doc.build(story)
            QMessageBox.information(self, "Sucesso", f"O relatório PDF foi salvo com sucesso em:\n{file_path}")
        except Exception as e:
            QMessageBox.critical(self, "Erro ao Salvar PDF", f"Ocorreu um erro ao gerar o PDF: {e}")
    ...
    def generate_interaction_plot(self, df, response_var, response_var_display_name, tukey_summary):
        ...
        try:
            ...
            QMessageBox.critical(self, "Erro na Geração do Gráfico", f"Não foi possível gerar o Gráfico de Interação:\n{str(e)}")
            plt.close('all')
    ...
    def export_statistics_to_csv(self):
        ...
        if path:
            try:
                ...
                QMessageBox.critical(self, "Erro de Exportação", f"Ocorreu um erro ao salvar o arquivo CSV:\n{str(e)}")
    ...
    def load_video(self, source):
        ...
        if not self.cap.isOpened():
            QMessageBox.critical(self, "Erro", f"Não foi possível abrir a fonte de vídeo: {source}")
            self.video_path = None
    ...
    def stop_analysis(self):
        ...
        if self.total_seeds_detected > 0:
            ...
            self.update_statistics_table()
            self.update_reports_display()
            QMessageBox.information(self, "Análise Concluída", "A análise do vídeo foi concluída e os resultados foram salvos nos relatórios.")
    ...
    def detect_seeds_in_frame(self, frame):
        ...
        if needs_yolo and self.yolo_model is None:
            try:
                ...
                QMessageBox.critical(self, "Erro ao Carregar YOLO", f"Não foi possível carregar o modelo YOLO.\nErro: {e}")
                self.method_hsv_radio.setChecked(True)
                needs_yolo = False
    ...
    def save_project(self):
        ...
        if filename:
            ...
            project_data = {
                **self.initial_data, # Adiciona todos os dados da tela de setup
                'video_path': self.video_path,
                'seeds_per_meter': self.seeds_per_m_input.text(),
                'row_spacing': self.row_spacing_combo.currentText(),
                'pixels_per_cm': self.pixels_per_cm,
                'auto_save_seed_count': self.auto_save_seed_count,
    ...
    def load_project(self):
        ...
        if filename:
            try:
                ...
                # --- CORREÇÃO: Carrega todos os dados do projeto ---
                self.initial_data = project_data # Atualiza os dados iniciais
                ...
                # --- CORREÇÃO: Verifica se os botões de rádio existem antes de tentar usá-los ---
                if hasattr(self, 'method_hsv_radio') and self.detection_method == 'hsv':
                    self.method_hsv_radio.setChecked(True)
                elif hasattr(self, 'method_yolo_radio') and self.detection_method == 'yolo':
                    self.method_yolo_radio.setChecked(True)
                elif hasattr(self, 'method_combined_radio') and self.detection_method == 'combined':
                    self.method_combined_radio.setChecked(True)
                    
                self.saturation_value = project_data.get('saturation_value', 0)
                self.temperature_value = project_data.get('temperature_value', 0)
                # Carrega as notas para cada análise
                for res in self.analysis_results:
                    res.setdefault('notes', '')
                self.next_analysis_id = max([res['id'] for res in self.analysis_results] or [0]) + 1

                # --- CORREÇÃO: Atualiza a UI com os dados carregados ---
                # Acessa os atributos da MainWindow, que devem existir se o setup_ui foi chamado
                self.video_path_label.setText(os.path.basename(self.video_path) if self.video_path else "Nenhum vídeo selecionado")
                self.populate_initial_data() # Preenche os comboboxes
                self.seeds_per_m_input.setText(str(project_data.get('seeds_per_meter', '')))
                self.auto_save_count_input.setText(str(self.auto_save_seed_count))
                self.pixels_cm_display.setText(f"{self.pixels_per_cm:.2f}")
                
                # --- CORREÇÃO: Verifica se os sliders existem antes de tentar usá-los (já feito na fase anterior) ---
                if hasattr(self, 'saturation_slider'):
                    self.saturation_slider.setValue(self.saturation_value)
                    self.saturation_value_label.setText(f"{self.saturation_value}%")
                if hasattr(self, 'temperature_slider'):
                    self.temperature_slider.setValue(self.temperature_value)
                    self.temperature_value_label.setText(f"{self.temperature_value}K")
                ...
                # --- NOVO: Preenche a tabela de estatísticas com os dados carregados ---
                self.update_statistics_table()

                QMessageBox.information(self, "Sucesso", "Projeto carregado com sucesso!")
            except Exception as e:
                QMessageBox.critical(self, "Erro", f"Erro ao carregar o projeto: {e}")

    def load_demo_project(self):
        """Carrega um projeto de demonstração a partir de um arquivo JSON."""
        demo_path = os.path.join(self.project_dir, "Demo.json")
        if not os.path.exists(demo_path):
            QMessageBox.critical(self, "Erro", f"Arquivo de demonstração não encontrado em:\n{demo_path}")
            return
        ...
        try:
            ...
            self.video_path = project_data.get('video_path')
            self.seed_type_combo.setCurrentText(project_data.get('seed_type', 'Soja'))
            self.speed_combo.setCurrentText(str(project_data.get('planting_speed', 3)))
            self.seeds_per_m_input.setText(str(project_data.get('seeds_per_meter', 14)))
            self.tube_type_combo.setCurrentText(project_data.get('tube_type', 'Reto'))
            self.set_pixels_per_cm(project_data.get('pixels_per_cm', 29.80))
            self.analysis_results = project_data.get('analysis_results', [])
            self.next_analysis_id = max([res.get('id', 0) for res in self.analysis_results] or [0]) + 1
            ...
            self.update_reports_display()
            self.update_statistics_table()
            QMessageBox.information(self, "Sucesso", "Projeto de demonstração carregado com sucesso!")
        except Exception as e:
            QMessageBox.critical(self, "Erro", f"Erro ao carregar o projeto de demonstração: {e}")
    ...
    def update_finished_handler(self, success, message):
        ...
        if success:
            QMessageBox.information(self, "Atualização Concluída", message)
            # Recarrega o modelo YOLO se o download foi bem-sucedido
            self.load_yolo_model()
        else:
            QMessageBox.warning(self, "Atualização Falhou", message)
...
if __name__ == '__main__':
    ...
    def save_project(self, auto_save=False):
        ...
        if file_path:
            try:
                ...
                QMessageBox.critical(self, "Erro ao Salvar Projeto", f"Não foi possível salvar o projeto:\n{e}")
    ...
    def load_project_data(self, project_data):
        ...
        QMessageBox.information(self, "Projeto Carregado", f"Projeto '{self.setup_data['test_name']}' carregado com sucesso. Calibração e resultados restaurados.")


if __name__ == '__main__':
    app = QApplication(sys.argv)
    # Simulação da versão atual
    dialog = UpdateDialog(current_version=REMOTE_VERSION)
    dialog.exec()
    sys.exit(app.exec())
