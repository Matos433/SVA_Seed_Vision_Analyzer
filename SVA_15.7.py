    ...
    def download_finished(self, success, file_path, status_label, button):
        ...
        if success:
            ...
            # --- LÓGICA PARA O SOFTWARE (.exe) ---
                ...
                # --- MODIFICAÇÃO: Lógica de reinício automático REMOVIDA ---
                # Agora, tanto o modo .py quanto o .exe apenas mostrarão esta 
                ...
                # Mensagem atualizada (sem o ".py")
                msg_reiniciar.setText(f"O novo arquivo '{filename}' foi baixado na pasta SVA.\n\n"
                                      "Por favor, feche o programa e execute o novo arquivo manualmente.")
                
                msg_reiniciar.setIcon(QMessageBox.Icon.Information)               
                # Aplica o estilo escuro padrão
                msg_reiniciar.setStyleSheet(DARK_MESSAGE_BOX_STYLE)
                msg_reiniciar.exec()
...
class AboutDialog(QDialog):
    """Janela 'Sobre' com informações do criador."""
    def __init__(self, parent=None):
        super().__init__(parent)
    ...
    def setup_ui(self):
        ...
        # Tenta carregar uma foto 'author.png', senão usa um placeholder cinza
        photo_path = resource_path("img_criador.jpeg")     
        if os.path.exists(photo_path):
            pixmap = QPixmap(photo_path)
            # Arredondar a imagem (opcional, estilo moderno)
            img = QImage(photo_path).scaled(150, 150, Qt.KeepAspectRatioByExpanding, Qt.SmoothTransformation)
            out_img = QImage(150, 150, QImage.Format_ARGB32)
            ...
            self.photo_label.setPixmap(QPixmap.fromImage(out_img))
        else:
            # Placeholder genérico (Círculo cinza com texto)
            self.photo_label.setStyleSheet("background-color: #e2e8f0; border-radius: 75px; color: #64748b; font-weight: bold;")
            self.photo_label.setText("FOTO")

        layout.addWidget(self.photo_label, alignment=Qt.AlignCenter)

        # --- 2. Nome ---
        name_label = QLabel("Talisson Sáteles Matos")
        name_label.setFont(QFont("Segoe UI", 24, QFont.Bold))
        name_label.setStyleSheet("color: #1e40af; margin-top: 10px;")
        name_label.setAlignment(Qt.AlignCenter)
        layout.addWidget(name_label)

        # --- 3. Resumo do Currículo ---
        bio_text = QTextEdit()
        ...
        # Texto (HTML para formatação)
        resume_content = """
        <p align="justify">
        Engenheiro Agrícola e Ambiental | Especialista em Agricultura de Precisão & Gestão de Projetos
        Com 7 anos de trajetória no setor, transformo desafios do agronegócio em soluções tecnológicas de alto impacto. </b></p>
        <p align="justify">
        Sou graduado pela UFMT (Campus Sinop), com Especialização em Agricultura de Precisão e MBA em Gestão de Projetos. </b></p>
        <p align="justify">
        Minha expertise une o conhecimento de campo ao desenvolvimento de software, sendo o idealizador e desenvolvedor do SVA (Seed Vision Analyzer). </b></p>
        Foco na otimização de processos e na implementação de inovações que elevam a eficiência produtiva.
        Profissional com sólida experiência em agricultura de precisão e desenvolvimento de soluções 
        tecnológicas para o agronegócio.
        </p>
        """
        bio_text.setHtml(resume_content)
        bio_text.setFixedHeight(300) # Altura fixa para a caixa de texto
...
# --- NOVO: THREAD PARA CARREGAR O MODELO YOLO ---
class YoloLoader(QThread):
    """Worker para carregar o modelo YOLO em uma thread separada para não travar a UI."""
    # Sinal emite (objeto_do_modelo, mensagem_de_erro_string)
    finished = Signal(object, str) 

    def __init__(self, model_path, parent=None):
        super().__init__(parent)
        self.model_path = model_path
        print(f"[YoloLoader] Iniciando thread para carregar: {self.model_path}")

    def run(self):
        """Executa a importação e o carregamento do modelo na thread."""
        try:
            # A importação pode ser lenta, por isso é feita DENTRO da thread
            from ultralytics import YOLO
            
            if not os.path.exists(self.model_path):
                print(f"[YoloLoader] Erro: Arquivo do modelo não encontrado em {self.model_path}")
                self.finished.emit(None, f"Arquivo do modelo (best.pt) não encontrado em:\n{self.model_path}")
                return

            print(f"[YoloLoader] Carregando modelo... (Esta é a parte lenta)")
            model = YOLO(self.model_path) # A parte demorada
            
            print(f"[YoloLoader] Modelo carregado com sucesso.")
            self.finished.emit(model, "") # Sucesso, envia o objeto do modelo

        except Exception as e:
            print(f"[YoloLoader] Erro ao carregar modelo: {e}")
            self.finished.emit(None, f"Ocorreu um erro ao importar ou carregar o modelo YOLO.\n\nDetalhe: {e}")
# --- FIM NOVO ---
...
class MainWindow(QWidget):
    ...
    def __init__(self, initial_data=None):
        ...
        self.yolo_loader_thread = None # <-- ADICIONE ESTA LINHA
    ...
    def setup_ui(self):
        ...
        # --- NOVO: Configuração do botão Sobre ---
        self.about_btn = QPushButton("ℹ️ Sobre")
        self.about_btn.setStyleSheet(top_button_style) # Usa o mesmo estilo dos outros
        self.about_btn.clicked.connect(self.show_about)

        
        # Adicionando o botão Sobre ao layout
    ...
    def setup_analyze_tab(self):
       ...
       # Trocado de QGridLayout para QHBoxLayout para colocar em linha única
        ...
        # --- NOVO: Indicador de carregamento YOLO ---
        self.yolo_load_indicator = QProgressBar(self)
        self.yolo_load_indicator.setRange(0, 0) # Modo indeterminado (busy)
        self.yolo_load_indicator.setTextVisible(False)
        self.yolo_load_indicator.setFixedSize(60, 14) # Pequeno e discreto
        self.yolo_load_indicator.setStyleSheet("""
            QProgressBar { 
                border: 1px solid #94a3b8; 
                border-radius: 4px; 
                background-color: #f1f5f9;
        ...
        """)
        self.yolo_load_indicator.setVisible(False) # Começa oculto
        method_hbox_layout.addWidget(self.yolo_load_indicator)
        # --- FIM NOVO ---
    ...
    def import_statistics_from_csv(self):
        ...
        try:
            ...
            msg_box.setIcon(QMessageBox.Information)
            msg_box.setWindowTitle("Sucesso")
            msg_box.setText(f"{len(imported_analyses)} registros importados com sucesso do arquivo CSV.")
            msg_box.setStyleSheet(DARK_MESSAGE_BOX_STYLE)
            msg_box.exec()
            ...
            msg_box.setText(f"Ocorreu um erro grave ao ler o arquivo CSV:\n{str(e)}\n\nVerifique se o arquivo está no formato correto (separado por ';').")
            msg_box.setStyleSheet(DARK_MESSAGE_BOX_STYLE)
            msg_box.exec()
    ...
    def perform_statistical_analysis(self):
        ...
        try:
            ...
            try:
                ...
                msg_box = QMessageBox(self)
                msg_box.setIcon(QMessageBox.Critical)
                msg_box.setWindowTitle("Erro de Cabeçalho")
                msg_box.setText("Não foi possível encontrar as colunas 'Deliniamento' ou 'Vel. Esteira (km/h)' na tabela. A análise não pode continuar.")
                msg_box.setStyleSheet(DARK_MESSAGE_BOX_STYLE)
                msg_box.exec()
                return
            # --- FIM DA CORREÇÃO ---
            ...
            msg_box = QMessageBox(self)
            msg_box.setIcon(QMessageBox.Critical)
            msg_box.setWindowTitle("Erro na Análise")
    ...
    def save_anova_results_to_pdf(self):
        ...
        try:
            doc.build(story)
            msg_box = QMessageBox.information(self, "Sucesso", f"O relatório PDF foi salvo com sucesso em:\n{file_path}")
            # A linha abaixo não é necessária, pois QMessageBox.information já aplica o estilo e executa.
            # msg_box.setStyleSheet(DARK_MESSAGE_BOX_STYLE)
        except Exception as e:
    ...
    def generate_interaction_plot(self, df, response_var, response_var_display_name, tukey_summary):
        ...
        try:
            ...
            msg.setStyleSheet(DARK_MESSAGE_BOX_STYLE)
            # msg.exec() # .exec() já é chamado por QMessageBox.critical
            plt.close('all')
    ...
    def export_statistics_to_csv(self):
        ...
        if path:
            try:
                ...
                msg.setStyleSheet(DARK_MESSAGE_BOX_STYLE)
                # msg.exec() # .exec() já é chamado por QMessageBox.information

            except Exception as e:
                # --- CORREÇÃO (AttributeError) ---
                msg_box = QMessageBox(self)
                msg_box.setIcon(QMessageBox.Critical)
                msg_box.setWindowTitle("Erro de Exportação")
                msg_box.setText(f"Ocorreu um erro ao salvar o arquivo CSV:\n{str(e)}")
                msg_box.setStyleSheet(DARK_MESSAGE_BOX_STYLE)
                msg_box.exec()
    ...
    def start_analysis(self):
        ...
        # --- LÓGICA DE CARREGAMENTO YOLO MOVIDA PARA CÁ ---
        
        # 1. Se estivermos apenas continuando do pause, não precisa checar o YOLO.
        if self.pause_flag:
            self.pause_flag = False
            self._start_playing_internal()
            return

        # 2. Se for um novo início, reseta a análise primeiro.
        self.reset_analysis()
        
        # 3. Verifica se o YOLO é necessário e se já está carregado.
        self.detection_method = self._get_current_detection_method() # Usa o novo helper
        needs_yolo = self.detection_method in ["yolo", "combined"]

        # CASO A: YOLO não é necessário (HSV) OU o modelo já está carregado
        if not needs_yolo or self.yolo_model is not None:
            self._start_playing_internal()
            return

        # CASO B: YOLO é necessário, mas o modelo NÃO está carregado
        if needs_yolo and self.yolo_model is None:
            # Se a thread já estiver rodando (clique duplo?), não faz nada
            if self.yolo_loader_thread and self.yolo_loader_thread.isRunning():
                print("Aguardando carregamento do YOLO já em progresso...")
                return
            
            print("Iniciando carregamento do modelo YOLO em background...")
            model_path = os.path.join(self.project_dir, "SVA", "best.pt")
            
            # Cria e conecta a thread
            self.yolo_loader_thread = YoloLoader(model_path, self)
            
            # Desconecta qualquer conexão antiga para evitar chamadas duplas
            try: self.yolo_loader_thread.finished.disconnect() 
            except TypeError: pass # Ignora erro se não havia conexão
            
            # Conecta ao handler NOVO/MODIFICADO
            self.yolo_loader_thread.finished.connect(self.on_yolo_model_loaded_and_start) 
            
            self.yolo_load_indicator.setVisible(True) # MOSTRA A BARRA
            
            # Desabilita os botões para evitar troca durante o carregamento
            self.set_controls_enabled(False) # Usa o novo helper
            
            self.yolo_loader_thread.start() # Inicia a thread
            
            # NÃO CHAMA _start_playing_internal() AQUI. O handler 'finished' o fará.
    ...
    def detect_seeds_in_frame(self, frame):
        ...
        # from ultralytics import YOLO # <-- REMOVA OU COMENTE ESTA LINHA
        ...
        # Lógica para carregar o modelo YOLO apenas quando necessário (AGORA COM THREAD)

        # A verificação de carregamento foi movida para start_analysis
        if needs_yolo:
            # Se o método for YOLO mas o modelo não estiver pronto, 
            # a UI (start_analysis) deve ter impedido o play.
            # Se chegamos aqui, o modelo DEVE estar pronto.
            if self.yolo_model is None:
                # Isso não deveria acontecer se a lógica em start_analysis estiver correta,
                # mas é um fallback de segurança.
                print("AVISO: Detecção YOLO pulada, modelo não está carregado.")
                needs_yolo = False 
            else:
                # Modelo está pronto, a lógica de detecção (abaixo) rodará normalmente.
                pass
    ...
    def set_tube_type(self, text):
        ...
        self.tube_type = text
    # --- NOVO HELPER 1 ---
    def _get_current_detection_method(self):
        """Lê os QRadioButtons e retorna a string do método selecionado."""
        if self.method_hsv_radio.isChecked(): return "hsv"
        if self.method_yolo_radio.isChecked(): return "yolo"
        if self.method_combined_radio.isChecked(): return "combined"
        return "hsv" # Padrão

    # --- NOVO HELPER 2 ---
    def set_controls_enabled(self, enabled):
        """Habilita/Desabilita os controles principais durante o carregamento."""
        self.start_btn.setEnabled(enabled)
        self.pause_btn.setEnabled(enabled)
        self.reset_btn.setEnabled(enabled)
        self.save_analysis_btn.setEnabled(enabled)
        self.select_video_btn.setEnabled(enabled)
        self.method_yolo_radio.setEnabled(enabled)
        self.method_hsv_radio.setEnabled(enabled)
        self.method_combined_radio.setEnabled(enabled)
   ...
   # Linha 5575 (Substitua a função completa)
    def load_project(self):
        ...
        if filename:
            try:
                ...
                # --- CORREÇÃO: CARREGA O VÍDEO AUTOMATICAMENTE ---
                if self.video_path and os.path.exists(self.video_path):
                    print(f"Carregando vídeo do projeto: {self.video_path}")
                    # Esta é a linha que faltava:
                    self.load_video(self.video_path) 
                elif self.video_path:
                    # O caminho foi salvo, mas o arquivo não foi encontrado no local original
                    print(f"Aviso: O arquivo de vídeo '{self.video_path}' não foi encontrado.")
                    msg_box_video_warning = QMessageBox(self)
                    msg_box_video_warning.setWindowTitle("Vídeo Não Encontrado")
                    msg_box_video_warning.setText(f"O vídeo '{os.path.basename(self.video_path)}' não foi encontrado no caminho original:\n\n{self.video_path}\n\nPor favor, selecione-o manualmente se desejar analisá-lo.")
                    msg_box_video_warning.setIcon(QMessageBox.Warning)
                    msg_box_video_warning.setStyleSheet(DARK_MESSAGE_BOX_STYLE)
                    msg_box_video_warning.exec()
                    self.video_path_label.setText(f"NÃO ENCONTRADO: {os.path.basename(self.video_path)}")
    ...
    def load_yolo_model(self):
        ...
        # --- NOVO: FUNÇÃO PARA RECEBER O MODELO DA THREAD ---
        # --- NOVO: FUNÇÃO PARA RECEBER O MODELO DA THREAD E INICIAR O PLAYER ---
    def on_yolo_model_loaded_and_start(self, model_object, error_message):
        """Chamado quando a thread YoloLoader termina. Inicia o player se for sucesso."""
        print("[Main] Sinal 'on_yolo_model_loaded_and_start' recebido.")
        self.yolo_load_indicator.setVisible(False) # Esconde a barra
        self.yolo_loader_thread = None # Limpa a referência da thread
        
        # Reabilita os botões de método e controles
        self.set_controls_enabled(True) # Usa o novo helper

        if error_message:
            # Se deu erro, limpa o modelo e mostra o aviso
            self.yolo_model = None
            
            msg_box = QMessageBox(self)
            msg_box.setIcon(QMessageBox.Critical)
            msg_box.setWindowTitle("Erro de Modelo YOLO")
            msg_box.setText(error_message)
            msg_box.setStyleSheet(DARK_MESSAGE_BOX_STYLE) 
            msg_box.exec()
            
            # Força o retorno para o método HSV como fallback
            self.method_hsv_radio.setChecked(True)
            # NÃO inicia o player, pois deu erro.
        else:
            # Se teve sucesso, armazena o objeto do modelo
            self.yolo_model = model_object
            print("[Main] Modelo YOLO armazenado com sucesso.")
            
            # --- A MÁGICA ESTÁ AQUI ---
            # Agora que o modelo está carregado, inicia o player.
            self._start_playing_internal()
    # --- FIM NOVO ---
    ...
    def reset_analysis(self):
        ...
        if self.cap:
            # self.reset_model_state() # <-- CORREÇÃO: Linha REMOVIDA
                                       # Isso garante que o modelo YOLO permaneça 
                                       # carregado na memória após a primeira vez.
